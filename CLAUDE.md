...
- Sinon on récupère les nouveaux emails non lus (hors SPAM), on les concatène dans **un seul** prompt Gemini (`gemini-2.0-flash`) pour produire une synthèse globale, et on envoie **une seule** notification FCM. Ne pas revenir à un appel LLM par email : le batching est intentionnel (coût et latence).
